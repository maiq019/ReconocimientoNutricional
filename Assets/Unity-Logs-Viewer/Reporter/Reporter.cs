...
public class Reporter : MonoBehaviour
{
    ...
    void calculateCurrentLog()
    {
        ...
        string _filterText = "";
        if (filter) _filterText = filterText.ToLower();
        currentLog.Clear();
        ...
        if (collapse)
        {
            for (int i = 0; i < collapsedLogs.Count; i++)
            {
                ...
                if (log.logType == _LogType.Log && !showLog) continue;
                if (log.logType == _LogType.Warning && !showWarning) continue;
                if (log.logType == _LogType.Error && !showError) continue;
                if (log.logType == _LogType.Assert && !showError) continue;
                if (log.logType == _LogType.Exception && !showError) continue;

                if (filter)
                {
                    if (log.condition.ToLower().Contains(_filterText)) currentLog.Add(log);
                }
                else
                {
                    currentLog.Add(log);
        ...
        if (selectedLog != null)
        {
            ...
            if (newSelectedIndex == -1)
            {
                ...
                if (newSelectedIndex != -1) scrollPosition.y = newSelectedIndex * size.y;
            }
            else
            {
    ...
    void DrawInfo()
    {
        ...
        GUILayout.Label("Mem Usage Of Logs " + logsMemUsage.ToString("0.000") + " mb",
                        nonStyle,
        ...
        float _size = GUILayout.HorizontalSlider(size.x,
                                                 16,
                                                 64,
                                                 sliderBackStyle,
                                                 sliderThumbStyle,
    ...
    void drawInfo_enableDisableToolBarButtons()
    {
        ...
        if (GUILayout.Button(clearOnNewSceneContent,
        ...
        if (GUILayout.Button(showTimeContent,
        ...
        if (GUILayout.Button(showSceneContent,
        ...
        if (GUILayout.Button(showMemoryContent,
        ...
        if (GUILayout.Button(showFpsContent,
        ...
        if (GUILayout.Button(searchContent,
    ...
    void drawToolBar()
    {
        ...
        if (GUILayout.Button(collapseContent,
        ...
        if (showClearOnNewSceneLoadedButton
         && GUILayout.Button(clearOnNewSceneContent,
        ...
        if (showTimeButton
         && GUILayout.Button(showTimeContent,
    ...
    void DrawLogs()
    {
        ...
        for (int i = startIndex; (startIndex + index) < endIndex; i++)
        {
            ...
            if (beforeHeight > 0) countRect.y += 8; //i will check later why
            countRect.width = w;
            ...
            if (collapse) GUI.Label(countRect, log.count.ToString(), barStyle);
            GUILayout.EndHorizontal();
            index++;
        }
    ...
    void drawGraph()
    {
        ...
        if (graphScrollerPos.x >= (samples.Count * graphSize - Screen.width)) graphScrollerPos.x += graphSize;
    ...
    bool isGestureDone()
    {
        ...
        if (gestureLength > gestureBase && gestureSum.magnitude < gestureBase / 2)
        {
            gestureDetector.Clear();
            gestureCount++;
            if (gestureCount >= numOfCircleToShow) return true;
        }
    ...
    void AddLog(string condition, string stacktrace, LogType type)
    {
        ...
        {
            bool skip = false;
            if (log.logType == _LogType.Log && !showLog) skip = true;
            if (log.logType == _LogType.Warning && !showWarning) skip = true;
            if (log.logType == _LogType.Error && !showError) skip = true;
            if (log.logType == _LogType.Assert && !showError) skip = true;
            if (log.logType == _LogType.Exception && !showError) skip = true;
        ...
        if (newLogAdded)
        {
            ...
            int totalCount = currentLog.Count;
            int totalVisibleCount = (int) (Screen.height * 0.75f / size.y);
            if (startIndex >= (totalCount - totalVisibleCount)) scrollPosition.y += size.y;
        }
    ...
    {
        if (clearOnNewSceneLoaded) clear();
    ...
    {
        string prefFile = "_build_info.txt";
        string url = prefFile;
