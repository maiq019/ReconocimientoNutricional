...
namespace Proxima
{
    internal class ProximaGameObjectCommands
    {
        ...
        {
            return !go.hideFlags.HasFlag(HideFlags.HideInHierarchy);
        }
        ...
        private static GameObject GetNextGameObject(GameObject go)
        {
            ...
            if (go != null)
            {
                ...
                if (go.scene != _dontDestroyOnLoadScene)
                {
                    var nextSceneIndex = 0;
                    for (int i = 0; i < SceneManager.sceneCount; i++)
                    {
                        if (SceneManager.GetSceneAt(i) == go.scene)
                        {
                            nextSceneIndex = i + 1;
                        }
                    }
                    ...
                    while (nextSceneIndex < SceneManager.sceneCount)
                    {
                        ...
                        if (nextScene.isLoaded)
                        {
                            ...
                            nextRoot = GetNextRootGameObject(0);
                            if (nextRoot != null)
                            {
                                return nextRoot;
                            }
                    ...
                    if (_dontDestroyOnLoadScene != null && go.scene != _dontDestroyOnLoadScene)
                    {
            ...
            _order = 0;
            SceneManager.GetSceneAt(0).GetRootGameObjects(_rootGameObjects);
            return GetNextRootGameObject(0);
        }
