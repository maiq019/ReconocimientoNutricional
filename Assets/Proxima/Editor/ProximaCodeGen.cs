...
namespace Proxima
{
    internal static class ProximaCodeGen
    {
        ...
        private static string GeneratePropertyInfo(Type componentType, string prop, string name)
        {
            ...
            {
                s += $"Updater = (Component o, ref object v) => {{ var x = (int)(({name})o).{prop}; if (!x.Equals((int)v)) {{ v = x; return true; }} return false; }} ";
            }
            else if (propertyType.IsValueType)
            {
                s += $"Updater = (Component o, ref object v) => {{ var x = (({name})o).{prop}; if (!x.Equals(({propertyTypeName})v)) {{ v = x; return true; }} return false; }} ";
            }
